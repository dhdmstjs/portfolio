function RadarChart(id, data, options) {
	...
	data.forEach(function(d){
		...
		d[value + "Average"] = d3.mean(d.values, function(e){
			...
			return e[value] });
		// console.log("dvalue", d[value + "Average"]);
	})
	...
	// 		//  console.log("format",Format(maxValue * d/cfg.levels));
	...
	blobWrapper
		.append("path")
		.attr("class", function(d) {return "radarArea" + " " + d[0].name.replace(/\s+/g, '') //Remove spaces from the areaName string to make one valid class name
		})
	...
	function cellover(d) {
			...
			//Bring back the hovered over blob
			d3.select("." + data[d][0].name.replace(/\s+/g, ''))
				.transition().duration(200)
